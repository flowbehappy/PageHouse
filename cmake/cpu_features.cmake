...
if (ARCH_AARCH64)
    ...
    option (PAGEHOUSE_ENABLE_ASIMD_SUPPORT "Enable Advanced SIMD support." ON)
    option (PAGEHOUSE_ENABLE_SVE_SUPPORT "Enable Scalable Vector Extension support." OFF)
    ...
    if (NO_ARMV81_OR_HIGHER)
        ...
        if (PAGEHOUSE_ENABLE_ASIMD_SUPPORT)
            ...
            add_definitions(-DPAGEHOUSE_ENABLE_ASIMD_SUPPORT=1)
        endif ()
    else ()
        ...
        if (PAGEHOUSE_ENABLE_SVE_SUPPORT)
            set (TEST_FLAG "${TEST_FLAG}+sve")
            add_definitions(-DPAGEHOUSE_ENABLE_SVE_SUPPORT=1)
        endif ()
...
elseif (ARCH_AMD64)
    ...
    option (PAGEHOUSE_ENABLE_AVX_SUPPORT "Use AVX/AVX2 instructions on x86_64" ON)
    option (PAGEHOUSE_ENABLE_AVX512_SUPPORT "Use AVX512 instructions on x86_64" ON)
...
#    option (NO_AVX_OR_HIGHER "Disable AVX or higher on x86_64 for maximum compatibility with older/embedded hardware." OFF)
    ...
    if (NO_AVX_OR_HIGHER)
        ...
        SET (PAGEHOUSE_ENABLE_ARCH_HASWELL_SUPPORT OFF)
    endif()
    ...
    set (PAGEHOUSE_COMPILER_AVX2_FLAG "-mavx2")
    set (TEST_FLAG "${PAGEHOUSE_COMPILER_AVX2_FLAG}")
    set (CMAKE_REQUIRED_FLAGS "${TEST_FLAG} -O0")
    check_cxx_source_compiles("
        #include <immintrin.h>
        int main() {
    ...
    " HAVE_AVX2)
    if (HAVE_AVX2 AND PAGEHOUSE_ENABLE_AVX_SUPPORT)
        # set (COMPILER_FLAGS "${COMPILER_FLAGS} ${TEST_FLAG}")
        add_definitions(-DPAGEHOUSE_ENABLE_AVX_SUPPORT=1)
    endif ()
    ...
    if (HAVE_AVX512 AND PAGEHOUSE_ENABLE_AVX512_SUPPORT)
        # set (COMPILER_FLAGS "${COMPILER_FLAGS} ${TEST_FLAG}")
        add_definitions(-DPAGEHOUSE_ENABLE_AVX512_SUPPORT=1)
    endif ()

    set (PAGEHOUSE_COMPILER_ARCH_HASWELL_FLAG "-march=haswell")
    check_cxx_compiler_flag("${PAGEHOUSE_COMPILER_ARCH_HASWELL_FLAG}" COMPILER_SUPPORT_ARCH_HASWELL)
